...
static const struct FFJniField jni_amediacodec_mapping[] = {
	{ "android/media/MediaCodec", NULL, NULL, FF_JNI_CLASS, offsetof(struct JNIAMediaCodecFields, mediacodec_class), 1 },
		...
		{ "android/media/MediaCodec", "reset", "()V", FF_JNI_METHOD, offsetof(struct JNIAMediaCodecFields, reset_id), 1 },
...
char *ff_AMediaCodecList_getCodecNameByType(const char *mime, int profile, int encoder, void *log_ctx)
{
	...
	jobjectArray colorformats = NULL;
	int colorcount = 0;
	...
	for (i = 0; i < codec_count; i++) {
		...
		for (j = 0; j < type_count; j++) {
			...
			LOGE("support type %s \r\n", supported_type);
			if (!strcasecmp(supported_type, mime)) {
				...
				colorformats = (*env)->GetObjectField(env, capabilities, jfields.color_formats_id);
				if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
					goto done;
				}
				colorcount = (*env)->GetArrayLength(env, colorformats);
				LOGE("color count %d \r\n", colorcount);
				if (colorcount)
				{
					jintArray cs = (jintArray)colorformats;
					jint* p = (*env)->GetIntArrayElements(env, cs, 0);
					for (k = 0; k < colorcount; k++)
					{
						LOGE("support color %d \r\n", p[k]);
					}
					(*env)->ReleaseIntArrayElements(env, cs, p, 0);
...
FFAMediaFormat *ff_AMediaFormat_new(void)
{
	...
	format = mallocz(sizeof(FFAMediaFormat));
	if (!format) {
		return NULL;
	}
...
FFAMediaCodec* ff_AMediaCodec_createCodecByName(const char *name)
{
	...
	codec = mallocz(sizeof(FFAMediaCodec));
	if (!codec) {
		return NULL;
	}
...
int ff_AMediaCodec_reset(FFAMediaCodec* codec)
{
	int ret = 0;
	JNIEnv *env = NULL;
	...
	(*env)->CallVoidMethod(env, codec->object, codec->jfields.reset_id);
	if (ff_jni_exception_check(env, 1, codec) < 0) {
		ret = AVERROR_EXTERNAL;
		goto fail;
	}
